...
//creat logout to when suplier logut the dashboard
    ...
    public function viewnonregisteredSupplier($UserId = null) {
        helper('form'); //load form
        $Company = new NonRegisteredSupplierModel(); //to create object from the emp model   
        $UserId = (session()->get('UserId'));
        $data['list'] = $Company->joinBidders($UserId);

        return view('web/viewnonregisteredSupplier', $data);
    }
   ...
   public function viewdetailedBid($AId = null, $IFBId = null, $TId = null, $UserId = null) {
        ...
        $data['listP'] = $invitation->getinvitationDetails($AId,$UserId);
    ...
    public function viewAppliedBidders($AId = null, $TId = null, $UserId = null) {
        ...
        $data['list'] = $announcement->getAnnounceDetails($AId);
        $data['listP'] = $invitation->getinvitationDetailssys($AId);
        $data['listB'] = $Bidders->GetaceptbidderDetails($AId, $TId);
    ...
    public function viewtenders($AId = null, $TId = null) {
        if (session()->get('isLoggedIn') === true) {
    ...
    public function edittenders($AId = null, $TId = null, $UserId = null) {
        ...
        $NSPData = new ShopingModel();
        $data['list'] = $sannouncement->getSAnnounceDetails($AId);
        $data['listtP'] = $NSPData->getinvitationDetailsbyTId($AId, $TId);
        echo $NSPData->getLastQuery()->getQuery();
      ...
      public function viewBid($IFBId = null, $TId = null) {
            helper('form');
            $data = array();
            $announcement = new AnnouncementModel();
            $invitation = new InvitationModel();
        ...
        if (session()->get('isLoggedIn') === true) {
            ...
            if ($this->request->getMethod() == 'post') {
                ...
                foreach ($Unit_priceEV as $key => $value) {
                    //echo $OPrice[$key];
                    //echo $key;
                    $PSchedule->update($this->request->getPost('id'), [
                        'UserId' => session()->get('UserId'),
                        'AnnounceId' => $this->request->getPost('AnnounceId'),
                        'TenderId' => $this->request->getPost('TenderId'),
                        'Itemid' => $Itemid[$key],
            ...
            if ($this->request->getMethod() == 'post') {
                ...
                $ItemName = $this->request->getPost('ItemName');
                foreach ($BiddersOffer as $key => $value) {
                    $SSpec->update($this->request->getPost('id'), [
                        'UserId' => session()->get('UserId'),
                        'AnnounceId' => $this->request->getPost('AnnounceId'),
                        'IId' => $IId[$key],
                        'Item' => $Item[$key],
            ...
            if ($this->request->getMethod() == 'post') {
                ...
                foreach ($deliverydate as $key => $value) {
                    ...
                    $SDelivery->update($this->request->getPost('id'), [
                        'UserId' => session()->get('UserId'),
                        'AnnounceId' => $this->request->getPost('AnnounceId'),
                        'Item' => $key,
                        'TenderId' => $this->request->getPost('TenderId'),
...
//check
    ...
    public function save_nonregisteredSuppliercheck() {
        helper('form');
        $data = array();
        $SCategory = new SupplierCatergoryModel();
        $data['Category_list'] = $SCategory->findAll();
        if ($this->request->getMethod() == 'post') {
            $rules = [
                ...
                'profile_image1' => [
                    'label' => 'Image File',
                    'rules' => 'uploaded[profile1_image]'
                    . '|is_image[profile1_image]'
                    . '|mime_in[profile1_image,image/jpg,image/jpeg,image/gif,image/png,image/webp]'
                    . '|max_size[profile1_image,100]'
                    . '|max_dims[profile1_image,1024,768]',
                ],
            ];
            ...
            if (!$this->validate($rules)) {
                ...
                $newName = $profile_image->getRandomName();
                $profile_image->move('public/images/' . 'uploads', $newName);
                $profile1_image = $this->request->getFile('profile1_image');
                $newName1 = $profile_image->getRandomName();
                $profile1_image->move('public/images/' . 'uploads', $newName1);
                // Generate a unique verification code
                $verificationCode = bin2hex(random_bytes(16));
                $userreg = new UserModel();
                $userreg->save([
    ...
    public function save_registeredSupplier() {
        ...
        if ($this->request->getMethod() == 'post') {
            $rules = [
                ...
                'RegDOC' => [
                    'label' => 'PDF File',
                    'rules' => 'uploaded[RegDOC]|permit_empty'
                    . '|mime_in[RegDOC,application/pdf]'
                    . '|max_size[RegDOC,10000]',
                ],
            ];
        } else {
            ...
            if (!empty($RegDOC->getName())) {
                $newDoc = $RegDOC->getRandomName();
                $RegDOC->move('public/images/uploads', $newDoc);
            }
