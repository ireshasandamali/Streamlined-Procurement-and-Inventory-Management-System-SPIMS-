...
class Sys extends BaseController {
    ...
    $data['listPp'] = $procurementPlan->findAll();
    
     $vote = new VoteModel();
     //use befre pesentage rv task
         //$data['listP'] = $vote->getvotebalanceaspercentage();
          $data['listP'] = $vote->getvotebalanceaspercentagervtask();
//echo $vote -> getLastQuery()->getQuery();
    ...
    //not used
     public function dashboardold() {
        ...
        //to display bids relevent to userID use later this code
        /*$SubmitedBid = new SubmitedBidModel();
        if($user_type == strtolower('supplier')){
        $data['bids'] =$SubmitedBid->where('UserId',session()->get('UserId'))->findAll();
        }
        if($user_type == strtolower('employee')){
        $data['bids'] =$SubmitedBid->where('bidStatus','Open')->findAll();
        }*/
        // get employee data
         //notification use this later ask the error
      ...
      //  $data['tender_Status'] = $req->where(["STATUS" => 1, "createTenderStatus" => 3])->findAll();
        ...
        $data['listB'] = $nonregSuppliers->findAll();
        $nonregSuppliers = count($data['listB']);
        //i think this no nedd after re viva
        //$data['nonregSuppliers'] = $nonregSuppliers;
