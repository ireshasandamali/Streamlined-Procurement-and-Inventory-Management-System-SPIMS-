        ...
        <!-- End navigation Title -->
            ...
            <div class="card" style="margin-top: 5px; margin-left: 25px;margin-right: 25px;"> 
                ...
                <div class="card-body">
                    ...
                    <?php
                    foreach ($listtP as $key => $resulttP) {
                        ?>
                        <h4 _ngcontent-c24="" class="society-section-header"><?= $resulttP['Description'] ?></h4><div _ngcontent-c24=""><hr>

                            <?php
                        }
                        ?>                     
                        <?= form_open_multipart('web/updatetenders',['novalidate'=> 'novalidate']) ?>
                            ...
                            <table class="table table-striped  ">
                                <tbody>
                                    ...
                                    foreach ($list as $key => $result) {
                                        ?>  
                                            <td> <strong><input type="hidden" name="AnnounceId" value="<?= $result['ids'] ?>">BID Validation Period</strong> </td>
                                            <td> <?= $result['BStarts'] ?> To  <?= $result['BEnds'] ?></td>
                                        </tr>
                                        <tr>
                                   ...
                                   foreach ($listtp as $key => $resulttP)  {
                                        ?>
                                        <tr>
                                            <td> <strong><input type="hidden" name="TenderId" value="<?= $resulttP['TId'] ?>">Procurement Number</strong> </td>
                                            <td>STF/LOG/C/<?= $resulttP['TId'] ?>/<?= substr($resulttP['BStartDate'],0,4) ?>  </td>
                                        </tr>
                                        <tr>
                                            <td> <strong> Title of the Procurement</strong> </td>
                                            <td><?= $resulttP['Description'] ?> </td>
                                        </tr>
                                        <tr>
                                            <td> <strong> Geographic Location (for the delivery of goods)</strong> </td>
                                            <td><?= $resulttP['Location'] ?> </td>
                                        </tr>
                                    ...
                                    <tbody  style="text-align: center; vertical-align: middle;">
                                        ...
                                        foreach ($listP as $key => $resultP) {
                                            ...
                                            <tr>
                                                ...
                                                <td><?= $resultP['ItemName'] ?> </td>                   
                                                <td><input type="hidden" name="Qty[<?= $resultP['TItemid'] ?>]" value="<?= $resultP['Qty'] ?>" id="qty<?= $resultP['TItemid'] ?>"><input type="hidden" name="ItemName[<?= $resultP['TItemid'] ?>]" value="<?= $resultP['ItemName'] ?>" id="ItemName<?= $resultP['TItemid'] ?>"><?= $resultP['Qty'] ?></td>
                                                <td><input type="hidden" name="Sample[<?= $resultP['TItemid'] ?>]" value="<?= $resultP['Sample'] ?>"><input type="hidden" name="Itemid[<?= $resultP['TItemid'] ?>]" value="<?= $resultP['Item'] ?>"><?= $resultP['Sample'] ?></td>                                           
                                                <td><input type="text" class="form-control" id="Unit_priceEV<?= $resultP['TItemid'] ?>" name="Unit_priceEV[<?= $resultP['TItemid'] ?>]" onchange="PriceLItem(<?= $resultP['TItemid'] ?>)" value="<?= set_value('Unit_priceEV',$resultP['Unit_priceEV']) ?>"</td>
                                                <td><input type="text" class="form-control" id="PriceLItem<?= $resultP['TItemid'] ?>" name="PriceLItem[<?= $resultP['TItemid'] ?>]" value="<?= set_value('PriceLItem',$resultP['PriceLItem']) ?>"</td>
                                                <td><input type="text" class="form-control" id="OPrice<?= $resultP['TItemid'] ?>" name="OPrice[<?= $resultP['TItemid'] ?>]"onchange="TotalPEVat(<?= $resultP['TItemid'] ?>)" value="<?= set_value('OPrice',$resultP['OPrice']) ?>"</td>
                                                <td><input type="text" class="form-control" id="TotalPEVat<?= $resultP['TItemid'] ?>" name="TotalPEVat[<?= $resultP['TItemid'] ?>]" value="<?= set_value('TotalPEVat',$resultP['TotalPEVat']) ?>"</td> 
                                                <td><input type="text" class="form-control" id="Discount<?= $resultP['TItemid'] ?>" name="Discount[<?= $resultP['TItemid'] ?>]"onchange="TotalwithDiscount(<?= $resultP['TItemid'] ?>)" value="<?= set_value('Discount',$resultP['Discount']) ?>"</td>
                                                <td><input type="text" class="form-control" id="TotalwithDiscount<?= $resultP['TItemid'] ?>" name="TotalwithDiscount[<?= $resultP['TItemid'] ?>]" value="<?= set_value('TotalwithDiscount',$resultP['TotalwithDiscount']) ?>"</td>
                                                <td><input type="text" class="form-control" id="VAT<?= $resultP['TItemid'] ?>" name="VAT[<?= $resultP['TItemid'] ?>]"onchange="VatAmount(<?= $resultP['TItemid'] ?>);TotalPIVat(<?= $resultP['TItemid'] ?>)" value="<?= set_value('VatAmount',$resultP['VatAmount']) ?>"</td>
                                                <td><input type="text" class="form-control" id="VatAmount<?= $resultP['TItemid'] ?>" name="VatAmount[<?= $resultP['TItemid'] ?>]" value="<?= set_value('VatAmount',$resultP['VatAmount']) ?>"</td>
                                                <td><input type="text" class="form-control" id="TotalPIVat<?= $resultP['TItemid'] ?>" name="TotalPIVat[<?= $resultP['TItemid'] ?>]" value="<?= set_value('TotalPIVat',$resultP['TotalPIVat']) ?>"</td>
                                            </tr>
                                            <?php
                                            $i++;
                                        }
                            ...
                            <table class="table">
                                ...
                                <tbody>
                                    ...
                                    foreach ($lists as $key => $results) {
                                        ...
                                        <tr>
                                            <th scope="row"><?= $i ?> </th>
                                            <td><input type="hidden" name="TenderId" value="<?= $results['TenderId'] ?>"><input type="hidden" name="Items" value="<?= $results['Item'] ?>"><input type="hidden" name="IId[<?= $results['SpecId'] ?>]"  value="<?= $results['IId'] ?>"><?= $results['ItemName'] ?></td>
                                            ...
                                            <td><select class="form-control" id="Cost" name="BiddersOffer[<?= $results['SpecId'] ?>]">
                                                     <option value="Yes"> <?= $results['BiddersOffer'] ?></option>
                                                    <option value="Yes">Yes</option>
                                                    <option value="No">No</option>
                                                </select></td>
                                            <td><textarea  class="form-control col-sm-10" id="Remarks" name="Remarks[<?= $results['SpecId'] ?>]" ><?= set_value('Remarks',$results['Remarks']) ?></textarea></td>                   
                                        </tr>
                                        <?php
                                        $i++;
                                    }
                                ...
                                <table class="table table-sm table-striped table-coler-#b3d9ff table-bordered">
                                    ...
                                    <tbody  style="text-align: center; vertical-align: middle;">
                                        ...
                                        foreach ($listG as $key => $resultG) {
                                            ...
                                            <tr>
                                                <th scope="row"><?= $i ?> </th>
                                                <td><input type="hidden" name="TenderId" value="<?= $resultG['TId'] ?>"><?= $resultG['ItemName'] ?> </td>                   
                                                <td><input type="hidden" name="Qty[<?= $resultG['Item'] ?>]" value="<?= $resultG['Qty'] ?>" id="qty<?= $resultG['Item'] ?>"><?= $resultG['Qty'] ?></td>
                                                <td><input type="hidden" name="Location[<?= $resultG['Item'] ?>]" value="<?= $resultG['Location'] ?>"><?= $resultG['Location'] ?></td>
                                                <td> <input type="text" class="form-control" id="EDate" name="EDate[<?= $resultG['Item'] ?>]" placeholder='30 days'></td>
                                                <td> <input type="text" class="form-control" id="LDate" name="LDate[<?= $resultG['Item'] ?>]" placeholder='90 days'></td>
                                                <td> <input type="text" class="form-control" id="deliverydate" name="deliverydate[<?= $resultG['Item'] ?>]" value="<?= set_value('deliverydate',$resultG['deliverydate']) ?>"</td>
                                             </tr>
                                            <?php
                                            $i++;
                                        }
...
<!--end of List of Goods & Delivery schedule -->
 ...
 <input type="hidden" class="form-control" id="id" name="id" value="<?= set_value('id',session()->get('UserId')) ?>">
