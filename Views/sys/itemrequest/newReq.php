<main id="main" class="main">
    <div class="pagetitle">
        <div class="row">

            <hr style="background-color: black; height: 2px;">
            <div class="col-8">
                <h1><img style="margin-right: 10px;" src="data:image/png;base64,"/>CREATE REQUEST</h1>       
            </div>
            <div class="col-4">
                <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">                       
                    <ol class="breadcrumb">
                        <img style="margin-right: 10px;" src="data:image/png;base64,"/>
                        <li class="breadcrumb-item"><a href="index.html">ITEM REQUEST</a></li>
                        <li class="breadcrumb-item active" aria-current="page"><img  style="margin-right: 10px;"  src="data:image/png;base64,"/>Create Request</li>
                    </ol>
                </nav>   
            </div>
            <hr style="background-color: #00001a; height: 2px;">
        </div>
        <!-- End Page Title -->

    <section class="section dashboard">        
         <div class="card" style="margin-top: 10px;"> 
            <div class="card-header" style="background-color: #7070db; color: white;">Create Item Request</div>
            <div class="card-body">
                  
                <!-- Alert Messages -->
                    <?php if (isset($msg)): ?>
                        <div class="alert alert-primary alert-dismissible fade show" role="alert">
                            <?= $msg; ?>
                            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                        </div>
                    <?php endif; ?>
                    <!-- End of Alert Messages -->
          <?= form_open_multipart('itemRequest/newReq')?>
            <div class="row g-3" style="margin-top: 2px">
                <div class="row mb-3">
                    <label for="ItemName" class="col-sm-6 col-form-label">Item Name</label>                
                    <input type="text" class="form-control col-sm-10" id="ItemName" name="ItemName">
                    <span class="text-danger"><?= service('validation')->getError('ItemName') ?></span>
                </div>
                 <div class="row mb-3">
                    <label for="Qty" class="col-sm-6 col-form-label">Quantity</label>                
                    <input type="text" class="form-control col-sm-10" id="Qty" name="Qty">
                    <span class="text-danger"><?= service('validation')->getError('Qty') ?></span>
                </div>
                
              
                <div class="row mb-3">
                    <label for="Purpose" class="col-sm-6 col-form-label">Purpose of the request</label>                 
                    <textarea  class="form-control col-sm-10" id="Purpose" name="Purpose" placeholder="Toners for Photocopy machine"></textarea>
                    <span class="text-danger"><?= service('validation')->getError('Purpose') ?></span>

                </div>
                 
                 <div class="row mb-3">
                 <label for="PStatus" class="col-sm-6 col-form-label">Priority Status</label>               
                     <select id="PStatus" class="form-select col-sm-10" name="PStatus">
                         <option value="" selected>...</option>
                         <option value="Urgent" <?php if (set_value('PStatus') == 'ICB') echo 'selected'; ?>>Urgent</option>
                         <option value="Priority" <?php if (set_value('PStatus') == 'Priority') echo 'selected'; ?>>Priority</option>
                         <option value="Normal" <?php if (set_value('PStatus') == 'Normal') echo 'selected'; ?>>Normal</option>
                     </select>
                     <span class="text-danger"><?= service('validation')->getError('PStatus') ?></span>                 
                 </div>
       
                <div class="row mb-3"> 
                    <label for="CName" class="col-sm-6 col-form-label">Camp</label>
                    <select id="CName" class="form-select col-sm-6 " name="CName">
                        <option value="" selected="">...</option>
                        <?php foreach ($camp_list as $key => $result) { ?>
                            <option value="<?= $result['CName'] ?>"><?= $result['CName'] ?></option>
                        <?php } ?>
                    </select>
                    <span class="text-danger"><?= service('validation')->getError('CName') ?></span>
                    <label for="ReqDate" class="col-sm-6 col-form-label">Request Date</label>
                    <input type="date" class="form-control col-sm-6 " id="ReqDate" name="ReqDate">
                    <span class="text-danger"><?= service('validation')->getError('ReqDate') ?></span>                 
                </div>
                 <div class="row mb-3">  
                      <label for="sample_image" class="col-sm-6 col-form-label">Sample Image</label>
                    <div class="col-sm-12">
                        <input class="form-control" type="file" id="sample_image" name="sample_image">
                    </div>
                </div>

                <div class="text-center">
                    <button  type="submit" class="btn btn-outline-primary rounded">Submit</button>
                    <button type="reset" class="btn btn-outline-primary rounded">Cancel</button>
                </div>

            </div>
        </div>          
        <?=form_close()?>
            
    </section>
</main>