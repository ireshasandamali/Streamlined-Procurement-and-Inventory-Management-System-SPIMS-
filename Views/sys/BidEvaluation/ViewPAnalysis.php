<main id="main" class="main">
    <div class="pagetitle">
        <div class="row">

            <hr style="background-color: black; height: 2px;">
            <div class="col-8">
                <h1><img style="margin-right: 10px;" src="data:image/png;base64,"/>TEC EVALUATION</h1>       
            </div>
            <div class="col-4">
                <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">                       
                    <ol class="breadcrumb">
                        <img style="margin-right: 10px;" src="data:image/png;base64,"/>
                        <li class="breadcrumb-item"><a href="index.html">Bid Evaluation</a></li>
                        <li class="breadcrumb-item active" aria-current="page"><img  style="margin-right: 10px;"  src="data:image/png;base64,"/>TEC Evaluation</li>
                    </ol>
                </nav>   
            </div>
            <hr style="background-color: #00001a; height: 2px;">
        </div>
        <!-- End Page Title -->

        <section class="section dashboard">        
            <div class="card" style="margin-top: 10px;"> 
                <div class="card-header" style="background-color: #7070db; color: white;">BID Evaluation Committee Members</div>
                <div class="card-body">
                    <h5 class="card-title"></h5>    
                    <!-- Multi Columns Form -->
                    <?= form_open_multipart('BidEvaluation/ViewPAnalysis') ?>
                    <div class="col-md-12">
                        <label for="business_name" class="form-label">Completeness of Bid submission form (Duly signed & filled)</label> 
                        <div class="d-flex">
                            <?php foreach ($listB as $key => $resultB) { ?>
                                <div class="card mx-3">
                                    <div class="card-body">
                                        <h5 class="card-title"><?= $resultB['business_name'] ?></h5>
                                       <?php foreach ($listD as $key => $resultD) {
                                                if ($resultD['fileType'] === 'BidSubmissionform') {
                                                    ?>
                                                    <div>
                                                        <a href="<?= site_url('public/images/uploads/' . $resultD['BidDOC']) ?>" target="_blank">View Documents</a>
                                                    </div>
                                                <?php }
                                            }
                                            ?>

                                    </div>
                                </div>
                            <?php } ?>
                        </div>
                    </div>


                    <!-- End Table Variants -->

                    <?= form_close() ?>
                </div>
            </div>
    </div>

</section>
</div>
</main>