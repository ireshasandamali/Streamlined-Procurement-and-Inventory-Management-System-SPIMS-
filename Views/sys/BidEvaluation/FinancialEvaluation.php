                                ...
                                <tbody>
                                   ...
                                   // print_r($listItem);
                                    foreach ($listItem as $key => $resultItem) {
                                        ?>
                                        <tr>
                                            <th colspan="2" style="text-align:center"><?= $resultItem['Item'] ?>-<?= $resultItem['ItemName'] ?></th>
                                         <th colspan="10" style="text-align:left"><?= $resultItem['UCost'] ?></th>
                                        ...
                                        <?php foreach ($BidderList as $key => $result) { ?>
                                            <?php foreach ($result as $key => $resulC) { ?>
                                               <?php if ($resulC['Itemid'] == $resultItem['Item']) { ?>
                                                    <tr>
                                                        <td><input type="hidden" class="form-control" name="CompanyName[<?= $resulC['UserId'].$resultItem['Item'] ?>]" value="<?= $resulC['business_name'] ?>"><?= $resulC['business_name'] ?></td>
                                                        <td><input type="hidden" class="form-control" name="Qty[<?= $resulC['UserId'].$resultItem['Item'] ?>]" value="<?= $resulC['Qty'] ?>"><?= $resulC['Qty'] ?></td>
                                                        
                                                        <td><input type="hidden" class="form-control" name="Itemid[<?= $resulC['UserId'].$resultItem['Item'] ?>]" value="<?= $resulC['Itemid'] ?>"><?= $resulC['Unit_priceEV'] ?></td>
                                                       <td><input type="hidden" class="form-control" name="Unit_priceEV[<?= $resulC['UserId'].$resultItem['Item'] ?>]" value="<?= $resulC['Unit_priceEV'] ?>"><?= $resulC['PriceLItem'] ?></td>
                                                        <td><input type="hidden" class="form-control" name="TotalPEVat[<?= $resulC['UserId'].$resultItem['Item'] ?>]" value="<?= $resulC['TotalPEVat'] ?>"><?= $resulC['OPrice'] ?></td>
                                                        <td><input type="hidden" class="form-control" name="VendorId[<?= $resulC['UserId'].$resultItem['Item'] ?>]" value="<?= $resulC['UserId'] ?>"><?= $resulC['TotalPEVat'] ?></td>
                                                        <td><input type="hidden" class="form-control" name="ItemName[<?= $resulC['UserId'].$resultItem['Item'] ?>]" value="<?= $resulC['ItemName'] ?>"><?= $resulC['Discount'] ?></td>
                                                        <td><?= $resulC['TotalwithDiscount'] ?></td>
                                                        <td><?= $resulC['VAT'] ?></td>                   
                                                        <td><?= $resulC['VatAmount'] ?></td>
                                                        <td><?= $resulC['TotalPIVat'] ?></td>
                                                        <td>
                                                            <input type="hidden" class="form-control" name="VendorId[<?= $resulC['UserId'].$resultItem['Item'] ?>]" value="<?= $resulC['UserId'] ?>">
                                                            <input type="text" class="form-control" id="marks" name="marks[<?= $resulC['UserId'].$resultItem['Item'] ?>]">
    ...
    <div class="tab-content">
        <div class="tab-pane fade show active" id="openedPreContent">
            ...
            <div class="card" style="margin-top: 10px;"> 
                ...
                <?php
                $i = 1;
                foreach ($listC as $key => $resultCo) {
                    ?>
                    <div class="card-body">
                        <h5 class="card-title">The below table summarizes the cost information of the bidders & estimated cost for the procurement of <strong> <?= $resultCo['Description'] ?> </strong> jointly conducted the bid evaluation and prepared the bid evaluation report. </h5>    
                    <?php } ?>
                    <div class="table-responsive">
                        <table class="table table-sm table-striped table-coler-#b3d9ff table-bordered">
                            <thead>
