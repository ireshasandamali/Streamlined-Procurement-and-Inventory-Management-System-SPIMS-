    ...
    <div class="pagetitle">
        ...
        <section class="section dashboard">   
            ...
            <div class="card" style="margin-top: 10px;"> 
                <div class="card-header" style="background-color: #00cc44; color: white;">THE PROCUREMENT COMMITTEE REPORT</div>
                <?php
                $i = 1;
                ...
                <div class="card-body" style="margin-left: 10px;margin-right: 15px">
                    ...
                    <?php } ?>
                    <!-- Multi Columns Form -->
                    <?= form_open_multipart('Procurement/Reports') ?>
                ...
                <div class="row mb-3">  
                  ...
                  <h5 class="card-title">Procurement Committee Aproval.</h5>   
                        ...
                        <?php foreach ($listcompany as $key => $resultcompany) { ?>
                            ...
                            foreach ($listfacept as $key => $resultfacept) {
                                if ($resultcompany['VendorId'] == $resultfacept['VendorId']) {
                                    ...
                                    <tr>                                           
                                        <td><!-- where is the vendor -->
                                                                                        <input type="hidden" name="CompanyName[<?= $resultcompany['VendorId'].$resultfacept['Itemid'] ?>]" value="<?= $resultcompany['CompanyName'] ?> ">
 <input type="hidden" name="CompanyName[<?= $resultcompany['VendorId'].$resultfacept['Itemid'] ?>]" value="<?= $resultcompany['CompanyName'] ?> ">
                                            <input type="hidden" name="ItemName[<?= $resultcompany['VendorId'].$resultfacept['Itemid'] ?>]" value="<?= $resultcompany['ItemName'] ?> ">
                                            <input type="hidden" name="VendorId[<?= $resultcompany['VendorId'].$resultfacept['Itemid'] ?>]" value="<?= $resultfacept['VendorId'] ?> "><input type="hidden" name="Itemid[<?= $resultcompany['VendorId'].$resultfacept['Itemid'] ?>]" value="<?= $resultfacept['Itemid'] ?> "><?= $i ?> </td>
                                        <td><?= $resultfacept['ItemName'] ?> </td>
                                        <td style="text-align: right"><input type="hidden" name="Qty[<?= $resultcompany['VendorId'].$resultfacept['Itemid'] ?>]" value="<?= $resultfacept['Qty'] ?> "><?= $resultfacept['Qty'] ?> </td>
                                        <td style="text-align: right"><input type="hidden" name="Unit_priceEV[<?= $resultcompany['VendorId'].$resultfacept['Itemid'] ?>]" value="<?= $resultfacept['Unit_priceEV'] ?> "><?= number_format($resultfacept['Unit_priceEV'], 2) ?></td>
                                        <td style="text-align: right"><input type="hidden" name="TotalPIVat[<?=$resultcompany['VendorId']. $resultfacept['Itemid'] ?>]" value="<?= $resultfacept['TotalPIVat'] ?> "><?= number_format($resultfacept['TotalPIVat'], 2) ?></td>
                ...
                <div class="row mb-3">  
                    <div class="col-sm-12">
                        ...
                        <?php
                                $i = 1;
                                foreach ($listpca as $key => $resultpmarks) { ?>
                                            
                                           <textarea id="Approval" name="Approval" rows="6" style="width: 100%; border: 1px solid #ccc;" readonly><?= $resultpmarks['Approval'] ?></textarea>
                                <?php } ?>
                        <span class="text-danger"><?= service('validation')->getError('Approval') ?></span>
                    </div>
                </div><!-- Multi Columns Form -->
            </div>
