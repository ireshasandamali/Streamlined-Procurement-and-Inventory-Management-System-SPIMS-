                ...
                <table class="table" id="ItemTable">
                    <thead>
                        <tr>
                            ...
                            <th scope="col">Add Request Amount</th>
                    ...
                    <tbody>
                        ...
                        foreach ($list as $key => $result) {
                            ...
                            <tr>
                                ...
                                <td><?= $result['Qty'] ?></td>
                                <td><input type="text" class="form-control RQty" oninput="BQty(this)" data-qty="<?= $result['Qty'] ?>"></td>
                                <td><input class="form-control PQty" type="text" readonly></td>
                                <td><a class="btn btn-success btn-sm" href="<?= site_url('inventory/viewbalance/' . $result['Itemid']) ?>">Check Availability of Item</a></td>
                            </tr>
                            <?php
                            $i++;
                        }
...
<script>
    function BQtych(input) {
        var Qty = parseFloat(input.getAttribute('data-qty'));
        var RQty = parseFloat(input.value);
        var PQty = Qty - RQty;
        var row = input.closest('tr');
        var PQtyInput = row.querySelector('.PQty');
        if (!isNaN(PQty) && PQty >= 0) {
            PQtyInput.value = PQty.toFixed(2);
        } else {
            PQtyInput.value = '';
        }
    }
...
<script>
    function BQtycheck(input) {
        ...
        if (!isNaN(PQty) && PQty >= 0) {
            ...
            PQtyInput.value = '';
            errorMessage.innerText = 'Requested quantity exceeds available quantity';
        }
    }
</script>
<script>
