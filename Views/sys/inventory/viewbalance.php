<main id="main" class="main">
    <div class="pagetitle">
        <div class="row">

            <hr style="background-color: black; height: 2px;">
            <div class="col-8">
                <h1><img style="margin-right: 10px;" src="data:image/png;base64,"/>Availability of Item</h1>       
            </div>
            <div class="col-4">
                <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">                       
                    <ol class="breadcrumb">
                        <img style="margin-right: 10px;" src="data:image/png;base64,"/>
                        <li class="breadcrumb-item"><a href="index.html">Inventory</a></li>
                        <li class="breadcrumb-item active" aria-current="page"><img  style="margin-right: 10px;"  src="data:image/png;base64,"/>Availability of Item</li>
                    </ol>
                </nav>   
            </div>
            <hr style="background-color: #00001a; height: 2px;">
        </div>
        <!-- End Page Title -->
    <section class="section dashboard">        
         <div class="card" style="margin-top: 10px;"> 
            <div class="card-header" style="background-color: #7070db; color: white;">Availability of Item</div>
            <div class="card-body">
                <h5 class="card-title"></h5>  
                
                <!-- Alert Messages -->
                    <?php if (isset($msg)): ?>
                        <div class="alert alert-primary alert-dismissible fade show" role="alert">
                            <?= $msg; ?>
                            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                        </div>
                    <?php endif; ?>
                    <!-- End of Alert Messages -->
                <?= form_open('inventory/viewbalance') ?>

                    <div class="row mb-4">
                    <div class="row mb-3">
                        <label for="ReqAmount" class="col-sm-6 col-form-label">Request Quantity</label>
                         <label for="ReqAmount" class="col-sm-6 col-form-label">Balance Quantity</label>
                          </div>
                        <div class="row mb-3">
                         <div class="col-sm-6">
                            <input type="text" class="form-control" id="ReqAmount" name="ReqAmount" oninput="BQty()">
                            <span class="text-danger"><?= service('validation')->getError('ReqAmount') ?></span>
                        </div>
                              <div class="col-sm-6">
                            <input type="text" class="form-control" id="BAmount" name="BAmount">
                            <span class="text-danger"><?= service('validation')->getError('BAmount') ?></span>
                        </div>
                    </div>
                   
                     <div class="text-center">
                        <button type="submit" class="btn btn-outline-primary rounded">Submit</button>
                        <button type="reset" class="btn btn-outline-primary rounded">Reset</button>
                    </div>

                    <?= form_close() ?><!-- End Horizontal Form -->

                </div>
            </div>
    </section>
</main>
<script>
    function BQty() {
        var EQty = parseFloat($('#quantity').val());
        var RQty = parseFloat($('#RQty').val());
        var PQty = EQty - RQty;
        if (!isNaN(PQty) && PQty >= 0) {
            $('#PQty').val(PQty.toFixed(2));
        } else {
            $('#PQty').val('');
        }
    }
</script>
