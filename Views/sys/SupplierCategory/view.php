<main id="main" class="main">
    <div class="pagetitle">
        <div class="row">

            <hr style="background-color: black; height: 2px;">
            <div class="col-8">
                <h1><img style="margin-right: 10px;" src="data:image/png;base64,"/>VIEW CATEGORY</h1>       
            </div>
            <div class="col-4">
                <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">                       
                    <ol class="breadcrumb">
                        <img style="margin-right: 10px;" src="data:image/png;base64,"/>
                        <li class="breadcrumb-item"><a href="index.html">SUPPLIER CATEGORY</a></li>
                        <li class="breadcrumb-item active" aria-current="page"><img  style="margin-right: 10px;"  src="data:image/png;base64,"/>View Category</li>
                    </ol>
                </nav>   
            </div>
            <hr style="background-color: #00001a; height: 2px;">
        </div>
        <!-- End Page Title -->
        <div class="card" style="margin-top: 10px;"> 
            <div class="card-header" style="background-color: #7070db; color: white;">Supplier Category</div>
            <div class="card-body">

                            <div class="card-body">
                                <h5 class="card-title"></h5> 
                        <!-- Multi Columns Form -->
                        <?= form_open('SupplierCategory/view') ?>

                        <div class="row g-3">
                            <div class="col-sm-2">
                                <label for="Vote" class="col-form-label">Supplier Category</label>
                            </div>
                            <div class="col-sm-4">
                                <input type="text" class="form-control" id="category" name="category">
                                <span class="text-danger"><?= service('validation')->getError('category') ?></span>
                            </div>
                            <div class="col-md-2">
                                <button style="border-radius: 40px;border: 1px solid  #000066"  type="submit" class="btn btn-primary">
                                    Search
                                    <img src="data:image/png;base64,"/>
                                </button>
                            </div>
                            <div class="col-md-2">
                                <a style="border-radius: 40px;border: 1px solid  #000066" type="reset" class="btn btn-secondary"href="<?= site_url('SupplierCategory/view') ?>">
                                    Reset
                                    <img src="data:image/png;base64,"/>
                                </a>
                            </div>
                        </div>
                        <!-- End Multi Columns Form -->

                        <?= form_close() ?>

                    </div>
                </div>
        </div>

                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title"></h5>
                        <!-- Table Variants -->
                        <table class="table table-bordered">
                            <thead>
                                <tr>
                                    <th scope="col">Category ID</th>
                                    <th scope="col">Category</th>
                                    <th scope="col">Description</th>
                                    <th scope="col">Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php
                                $i = 1;
                                foreach ($list as $key => $result) {
                                    ?>
                                    <tr>
                                        <th scope="row"><?= $i ?> </th>
                                        <td><?= $result['category'] ?></td>
                                        <td><?= $result['Description'] ?></td>
                                        <td>  <a type="button" class="btn btn-danger" href="javascript:void(0)" onclick="confirmDelete(<?= $result['categoryid'] ?>)"><i class="fa fa-trash" aria-hidden="true"></i>Delete</a>

                                            <!-- delete confirmation message -->


                                    </tr>
                                    <?php
                                    $i++;
                                }
                                ?>

                            </tbody>
                        </table>
                        <!-- End Table Variants -->

                    </div>
                </div>
                </section>
                </main>

                <script>
                    function confirmDelete(id) {
                        if (confirm("Are you sure, you want to delete this record?")) {
                            window.location.href = '<?= site_url("SupplierCategory/removeItem/") ?>' + id;
                        }
                    }
                </script>