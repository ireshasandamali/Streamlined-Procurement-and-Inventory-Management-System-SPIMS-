<main id="main" class="main">
    <div class="pagetitle">
        <div class="row">

            <hr style="background-color: black; height: 2px;">
            <div class="col-8">
                <h1><img style="margin-right: 10px;" src="data:image/png;base64,"/>VIEW CAMP</h1>       
            </div>
            <div class="col-4">
                <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">                       
                    <ol class="breadcrumb">
                        <img style="margin-right: 10px;" src="data:image/png;base64,"/>
                        <li class="breadcrumb-item"><a href="index.html">DIVISION</a></li>
                        <li class="breadcrumb-item active" aria-current="page"><img  style="margin-right: 10px;"  src="data:image/png;base64,"/>View Camp</li>
                    </ol>
                </nav>   
            </div>
            <hr style="background-color: #00001a; height: 2px;">
        </div>
        <!-- End Page Title -->
    <div class="card" style="margin-top: 10px;"> 
        <div class="card-header" style="background-color: #7070db; color: white;">SPECIAL TASK FORCE CAMP</div>
        <div class="card-body">
            <h5 class="card-title"></h5>                           
           
                    <!-- Multi Columns Form -->

                                <?= form_open('division/viewCamp') ?>
                                <div class="row g-3">                              
                                        <!-- check -->
                                        <!-- Add this code before the table -->                                      
                                            <div class="col-md-1">
                                             <label for="entries" class="form-label">Show </label>
                                            </div>
                                                <div class="col-md-2">
                                                <select style="border-radius: 40px;border: 1px solid  #000066" id="entries" class="form-select" onchange="changeEntries(this.value)">
                                                    <option value="5">5</option>
                                                    <option value="10" selected>10</option>
                                                    <option value="15">15</option>
                                                </select>
                                            </div>
                                        <div class="col-md-1">
                                             <label for="entries" class="form-label">Entries</label>
                                            </div>
                                                   <!-- check -->
                                    <div class="col-md-2">
                                        
                                            <input style="border-radius: 40px;border: 1px solid  #000066"  type="text" class="form-control" id="CName" name="CName" placeholder="Camp Name">
                                            <span class="text-danger"><?= service('validation')->getError('CName') ?></span>
                                       
                                    </div>
                                    <div class="col-md-2">
                                        
                                             <input style="border-radius: 40px;border: 1px solid  #000066"  type="text" class="form-control" id="CNo" name="CNo" placeholder="Camp Number">
                                             <span class="text-danger"><?= service('validation')->getError('CNo') ?></span>
                                   </div> 
                                   <div class="col-md-2">
                                       <button style="border-radius: 40px;border: 1px solid  #000066"  type="submit" class="btn btn-primary">
                                           Search
                                           <img src="data:image/png;base64,"/>
                                       </button>
                                   </div>
                                   <div class="col-md-2">
                                       <a style="border-radius: 40px;border: 1px solid  #000066" type="reset" class="btn btn-secondary"href="<?= site_url('division/viewCamp') ?>">
                                    Reset
                                    <img src="data:image/png;base64,"/>
                                       </a>
                                   </div>              
                                </div>
                                <!-- End Multi Columns Form -->
                                <?= form_close() ?>

                        
                    </div>
                   
                </div>
       

            <div class="card">
             <div class="card-header" style="background-color: #7070db; color: white;"> Number Of Camps <?= count($list) ?>  </div>
                <div class="card-body">
                    <h5 class="card-title"></h5>
                    <!-- Table Variants -->
                    <table id="campTable" class="table table-borderless datatable">
                    <thead style="vertical-align: middle; border-bottom: 2px solid #001a66; border-top:  2px solid #001a66">
                            <tr>
                          
                                <th scope="col">Camp Number</th>
                                <th scope="col">Camp Name</th>
                                <th></th>
                                    <?php if((session()->get('UserType'))=='Administrator'){ ?> 
                                <th  style="text-align: center">Action</th>
                                    <?php } ?>
                            </tr>
                        </thead>
                        <tbody>
                            <?php
                            
                            $i = 1;
                            foreach ($list as $key => $result) {
                               
                                ?>
                                <tr>
                                    
                                    <td><?= $result['CNo'] ?></td>
                                    <td><?= $result['CName'] ?></td>
                                        <?php if((session()->get('UserType'))=='Administrator'){ ?> 
                                    <td><a class="btn btn-primary" href="<?= site_url('division/editcamp/' . $result['id']) ?>"> Edit</a>
                                 <td>        
                                            <!-- delete confirmation message -->
                                            <input type="hidden" name="id" value="<?= $result['id'] ?>">
                                            <button type="button" class="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#deleteModal<?= $result['id'] ?>" onclick="confirmDeleteModal('<?= $result['id'] ?>')">Delete</button>

                                            <!-- Modal -->
                                            <div class="modal fade" id="deleteModal<?= $result['id'] ?>" tabindex="-1" role="dialog" aria-labelledby="deleteModalLabel<?= $result['id'] ?>" aria-hidden="true">
                                                <div class="modal-dialog" role="document">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <h5 class="modal-title" id="deleteModalLabel<?= $result['id'] ?>">Confirm Deletion</h5>
                                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                        </div>
                                                        <div class="modal-body" id="msg<?= $result['id'] ?>"></div>
                                                        <div class="modal-footer">
                                                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                                                            <a class="btn btn-danger" id="confirmDelete<?= $result['id'] ?>" href="<?= site_url('division/removeCamp/' . $result['id']) ?>">Delete</a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
                                        <?php } ?>
                                </tr>
                                <?php
                                $i++;
                            }
                            ?>
                        </tbody>
                    </table>
                    <!-- End Table Variants -->
                </div>
            </div>
        </div>
    </div>
</section>
</main>
<script>
    
     function confirmDeleteModal(Campid) {
                        // Construct the ID of the modal and its related elements using the Employee ID
                        var modalId = "#deleteModal" + Campid;
                        var msgId = "#msg" + Campid;
                        var confirmDeleteId = "#confirmDelete" + Campid;

                        // Set the item ID in the modal body
                        $(msgId).html("Are you sure you want to delete Camp ID: " + Campid + "?");

                        // Set the delete button's click event handler
                        $(confirmDeleteId).off("click").on("click", function () {
                            // Perform the deletion logic here
                            console.log("Deleting Camp ID: " + Campid);

                            // Close the modal
                            var deleteModal = bootstrap.Modal.getInstance(document.getElementById(modalId.substring(1)));
                            deleteModal.hide();

                          
                        });
                    }

    
    function changeEntries(entries) {
        // Get all table rows
        var rows = document.querySelectorAll('#campTable tbody tr');

        // Hide all rows
        rows.forEach(row => {
            row.style.display = 'none';
        });

        // Show the specified number of entries
        for (var i = 0; i < entries; i++) {
            if (rows[i]) {
                rows[i].style.display = '';
            }
        }
    }
</script>

