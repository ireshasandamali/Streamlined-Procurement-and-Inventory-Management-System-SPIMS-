<main id="main" class="main">
    <div class="pagetitle">
        <div class="row">
            <hr style="background-color: black; height: 2px;">
            <div class="col-8">
                <h1><img style="margin-right: 10px;" src="data:image/png;base64,"/>VIEW TENDER</h1>       
            </div>
            <div class="col-4">
                <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">                       
                    <ol class="breadcrumb">
                        <img style="margin-right: 10px;" src="data:image/png;base64,"/>
                        <li class="breadcrumb-item"><a href="index.html">TENDER</a></li>
                        <li class="breadcrumb-item active" aria-current="page"><img  style="margin-right: 10px;"  src="data:image/png;base64,"/>View Tender</li>
                    </ol>
                </nav>   
            </div>
            <hr style="background-color: #00001a; height: 2px;">
        </div><!-- End Page Title -->   

        <!-- Multi Columns Form -->

        <div class="card" style="margin-top: 10px;"> 
            <div class="card-header" style="background-color: #7070db; color: white;">BId Bond</div>
            <div class="card-body">
                <h5 class="card-title"></h5>           
                <div class="row">
                    <div class="col-md-12">
                        <div class="card" style="border: 2px solid  #ccd9ff; border-radius: 0;">
                            <div class="card-body">
                                <h5 class="card-title"></h5>
                                <!-- Alert Messages -->
                                <?php if (isset($msg)): ?>
                                    <div class="alert alert-primary alert-dismissible fade show" role="alert">
                                        <?= $msg; ?>
                                        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                                    </div>
                                <?php endif; ?>
                                <!-- End of Alert Messages -->
                                <?= form_open('Payments/BidBondAdd') ?>

                              <div class="row mb-3">
                                    <label for="Tenderid" class="col-sm-2 form-label">Tender Id Number</label>
                                    <div class="col-md-12">  

                                        <select id="Tenderid" class="form-select" name="Tenderid" onchange="loadDescription(this.value); loadBidders(this.value);">
                                            <option value="" selected="">...</option>
                                            <?php foreach ($Tender_list as $key => $result) { ?>
                                                <?php if ($result['PType'] == 'NCB'): ?>
                                                    <option value="<?= $result['Tenderid'] ?>">STF/LOG/C/<?= $result['Tenderid'] ?>/<?= substr($result['BStartDate'], 0, 4) ?></option>
                                                <?php endif; ?>
                                            <?php } ?>
                                            <span class="text-danger"><?= service('validation')->getError('Tenderid') ?></span>                   
                                        </select>
                                    </div> 

                                    <!--get Description from onchange-->   
                                    <div class="col-md-12">  
                                        <label for="Description" class="form-label">Tender Description</label>
                                        <input type="text" class="form-control" id="result" name="Description" >
                                    </div>
                                     <div class="row mb-3">   
                                           <label for="BidderList" class="form-label">Bidders List</label>
                                    <div class="col-sm-12"> 
                                        <div id="BidderListList">
                                            <select id="BidderList"  class="form-select" name="BidderList"  >

                                                <option selected="">...</option>
                                                <?php foreach ($Bidder_list as $key => $resultn) { ?>
                                                    <option value="<?= $resultn['VendorId'] ?>"><?= $resultn['CompanyName'] ?></option>
                                                <?php } ?>
                                            </select>
                                        </div>
                                        <span class="text-danger"><?= service('validation')->getError('BidderList') ?></span>
                                    </div>
                                </div>
                                    <div class="row mb-3">
                                        <label for="ADate" class="col-sm-6 col-form-label">Date of the Letter</label>
                                          <label for="LetterNo" class="col-sm-6 col-form-label">Purpose</label>
                                        
                                    </div>
                                    <div class="row mb-3">                               
                                        <div class="col-sm-6"> 
                                            <input type="date" class="form-control" id="ADate" name="ADate"  >
                                            <span class="text-danger"><?= service('validation')->getError('ADate') ?></span>
                                        </div>

                                        <div class="col-sm-6">
                                               <select id="LetterNo" class="form-select" name="LetterNo">
                                <option selected="selected">...</option>
                                <option value="Release Bid Bond" <?php
                                if (set_value('LetterNo') == 'Release Bid Bond') {
                                    echo 'selected';
                                }
                                ?> >Release Bid Bond</option>
                                <option value="Extend Bid Bond" <?php
                                if (set_value('LetterNo') == 'Extend Bid Bond') {
                                    echo 'selected';
                                }
                                ?> >Extend Bid Bond</option>
                                
                            </select>
                                            <span class="text-danger"><?= service('validation')->getError('LetterNo') ?></span>                                                                               
                                        </div>
                                    </div>
                                  
 
                                    <div class="text-center">
                                        <!-- use submit button -->
                                        <button type="button" class="btn btn-primary btn-sm"  onclick="constructURLAndRedirect()">Letter of Intent</button>
                                    </div>


                             
   </div>


                                <?= form_close() ?>

                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
</main>

<script>
    function  loadDescription(Tid) {
        //alert(Tid)
        var formData = "Tenderid=" + Tid + "&";
        //alert(formData);
        $.ajax({
            type: 'POST',
            url: '<?= site_url('Payments/getDescription') ?>',
            data: formData,
            success: function (response) {
                $("#result").val(response);
            },
            error: function (xhr, status, error) {
                alert(error);
            }
        });
    }
    
    
    
     function loadBidders(nid) {
        //alert(nid);
        var formData = "TenderId=" + nid + "&";
        $.ajax({
            type: 'POST',
            url: '<?= site_url('Payments/getBidderList') ?>',
            data: formData,
            success: function (response) {
                // alert(response);
                $("#BidderListList").html(response);
            },
            error: function (xhr, status, error) {
                alert(error);
            }
        });
    }
    
      function constructURLAndRedirect() {
        //alert('ok');
        var tenderId = document.getElementById('Tenderid').value;
        var vendorId = document.getElementById('BidderList').value;
        var ADate = document.getElementById('ADate').value;
      //  var PBond = document.getElementById('PBond').value;
       //  var PBond = document.getElementById('PBond').value;
       // var PBondl = document.getElementById('PBondl').value;
        var LetterNo = document.getElementById('LetterNo').value;
        //alert(vendorId);

        var url = "<?= site_url('Payments/createLetterofBbond/') ?>" + tenderId + '/' + vendorId + '/' + ADate + '/'  + LetterNo;
        //alert(url);
        window.location.href = url;
    }


    </script>