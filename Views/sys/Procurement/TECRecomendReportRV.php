<main id="main" class="main">
    <div class="pagetitle">
        <div class="row">
            <hr style="background-color: black; height: 2px;">
            <div class="col-8">
                <h1><img style="margin-right: 10px;" src="data:image/png;base64,"/>ASSIGN COMMITTEE</h1>       
            </div>
            <div class="col-4">
                <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">                       
                    <ol class="breadcrumb">
                        <img style="margin-right: 10px;" src="data:image/png;base64,"/>
                        <li class="breadcrumb-item"><a href="index.html">Bid Evaluation</a></li>
                        <li class="breadcrumb-item active" aria-current="page"><img  style="margin-right: 10px;"  src="data:image/png;base64,"/>Assign Committee</li>
                    </ol>
                </nav>   
            </div>
            <hr style="background-color: #00001a; height: 2px;">
        </div>
        <!-- End Page Title -->
    <section class="section dashboard">        
         <div class="card" style="margin-top: 10px;"> 
            <div class="card-header" style="background-color: #7070db; color: white;">Technical Evaluation Committee Report</div>
            <div class="card-body">
                <h5 class="card-title"></h5>    
                <!-- Multi Columns Form -->
                <?= form_open_multipart('Procurement/TECRecomendReport') ?>
                
               <div class="row g-3">
               <div class="table-responsive" style="margin-top: 20px;"> <!-- Added margin-top to create space -->
                        <table class="table table-sm table-striped table-coler-#b3d9ff table-bordered">
                            <thead>
                                <tr>
                                    <th scope="col" class="text-center align-middle">Meeting No</th>
                                    <th scope="col" class="text-center align-middle">Procurement No</th>
                                    <th scope="col" class="text-center align-middle">Tender Description</th>                                  
                                    <th scope="col" class="text-center align-middle">Meeting Date</th>
                                    <th scope="col" class="text-center align-middle">Action</th>
                                    
                                </tr>
                            </thead>

                            <tbody>
                                <?php
                                $i = 1;
                                foreach ($listC as $key => $resultC) {
                                    ?>
                                    <tr>
                                        <td style="font-size: 12px;"><input type="hidden" name="AnnounceId" value="<?= $resultC['PNumber'] ?>"><?= $resultC['Committeeid'] ?></td>
                                        <td style="font-size: 12px;">STF/LOG/C/<?= $resultC['PNumber'] ?>/<?= substr($resultC['BStartDate'],0,4) ?></td>
                                        <td style="font-size: 12px;"><?= $resultC['Description'] ?></td>
                                        <td style="font-size: 12px;"><?= $resultC['MDate'] ?></td>                                       
                                        <td><a class="btn btn-primary" type="button" href="<?= site_url('BidEvaluation/PreliminaryEvaluation/'.$resultC['PNumber']) ?>"style="color: white;">Open</a></td>                                                       
                                    </tr>
                                    <?php
                                    $i++;
                                }
                                ?>

                            </tbody>
                        </table>
                        <!-- End Table Variants -->
               
<?= form_close() ?>
</div>
            </div>
        </div>

    </section>
    </div>
</main>