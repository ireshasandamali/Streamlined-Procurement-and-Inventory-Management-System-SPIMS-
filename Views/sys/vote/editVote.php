<main id="main" class="main">
    <div class="pagetitle">
        <div class="row">

            <hr style="background-color: black; height: 2px;">
            <div class="col-8">
                <h1><img style="margin-right: 10px;" src="data:image/png;base64,"/>ADD VOTE</h1>       
            </div>
            <div class="col-4">
                <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">                       
                    <ol class="breadcrumb">
                        <img style="margin-right: 10px;" src="data:image/png;base64,"/>
                        <li class="breadcrumb-item"><a href="index.html">VOTE</a></li>
                        <li class="breadcrumb-item active" aria-current="page"><img  style="margin-right: 10px;"  src="data:image/png;base64,"/>Add Vote</li>
                    </ol>
                </nav>   
            </div>
            <hr style="background-color: #00001a; height: 2px;">
        </div><!-- End Page Title -->

    <section class="section dashboard">
        <div class="card" style="margin-top: 10px;"> 
            <div class="card-header" style="background-color: #7070db; color: white;">VOTE</div>
            <div class="card-body">
                <h5 class="card-title"></h5>
                <!-- Alert Messages -->
                    <?php if (isset($msg)): ?>
                        <div class="alert alert-primary alert-dismissible fade show" role="alert">
                            <?= $msg; ?>
                            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                        </div>
                    <?php endif; ?>
                    <!-- End of Alert Messages -->
                <?= form_open('vote/update') ?>
                <div class="row mb-4">
                    <div class="row mb-3">
                        <label for="Vote" class="col-sm-2 col-form-label">Vote Name</label>
                        <div class="col-sm-10">
                            <input type="text" class="form-control" id="Vote" name="Vote" value="<?= set_value('Vote',$vote['Vote']) ?>">
                            <span class="text-danger"><?= service('validation')->getError('Vote') ?></span>
                        </div>
                    </div>
                    <div class="row mb-3">
                        <label for="Description" class="col-sm-2 col-form-label">Description</label>
                        <div class="col-sm-10">
                            <input type="text" class="form-control" id="Description" name="Description" value="<?= set_value('Description',$vote['Description']) ?>">
                            <span class="text-danger"><?= service('validation')->getError('Description') ?></span>
                        </div>
                    </div>
                    <div class="row mb-3">
                        <label for="Allocation" class="col-sm-2 col-form-label">Allocation</label>
                        <div class="col-sm-10">
                            <input type="text" class="form-control" id="Allocation" name="Allocation" value="<?= set_value('Allocation',$vote['Allocation']) ?>">
                            <span class="text-danger"><?= service('validation')->getError('Allocation') ?></span>
                        </div>
                    </div>
                    <div class="text-center">
                        <input type="hidden" class="form-control" id="id" name="id" value="<?= set_value('id',$vote['id']) ?>">
                        <button type="submit" class="btn btn-outline-primary rounded">Submit</button>
                        <button type="reset" class="btn btn-outline-primary rounded">Reset</button>
                    </div>

                    <?= form_close() ?><!-- End Horizontal Form -->

                </div>
            </div>
            </div>
       
    </section>
</main>
