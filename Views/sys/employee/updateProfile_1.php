<main id="main" class="main">
    <div class="pagetitle">
        <div class="row">
            <hr style="background-color: black; height: 2px;">
            <div class="col-8">
                <h1><img style="margin-right: 10px;" src="data:image/png;base64,"/>VIEW EMPLOYEE</h1>       
            </div>
            <div class="col-4">
                <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">                       
                    <ol class="breadcrumb">
                        <img style="margin-right: 10px;" src="data:image/png;base64,"/>
                        <li class="breadcrumb-item"><a href="index.html">Employee</a></li>
                        <li class="breadcrumb-item active" aria-current="page"><img  style="margin-right: 10px;"  src="data:image/png;base64,"/>View Employee</li>
                    </ol>
                </nav>   
            </div>
            <hr style="background-color: #00001a; height: 2px;">
        </div>
        <!-- End Page Title -->

        <!-- updateProfile.php -->
        <section class="section dashboard">
            <div class="row">
                <div class="col-md-6">
                    <div class="card">
                        <div class="card-body">
                            <h5 class="card-title">Employee Details</h5>

                            <?php foreach ($list as $key => $result): ?>
                                <div class="form-group">
                                    <label for="employee_name" class="form-label">Name</label>
                                    <input type="text" class="form-control" id="employee_name" value="<?= $result['CivilStatus'] ?> . <?= $result['FirstName'] ?> <?= $result['LastName'] ?>" readonly>
                                </div>
                                <div class="form-group">
                                    <label for="employee_email" class="form-label">Email</label>
                                    <input type="email" class="form-control" id="employee_email" value="<?= $result['Email'] ?>" readonly>
                                </div>
                                <div class="form-group">
                                    <label for="employee_tel" class="form-label">Telephone Number</label>
                                    <input type="text" class="form-control" id="employee_tel" value="<?= $result['TelNo'] ?>" readonly>
                                </div>
                                <div class="form-group">
                                    <label for="employee_address" class="form-label">Address</label>
                                    <textarea class="form-control" id="employee_address" readonly><?= $result['Address'] ?></textarea>
                                </div>
                            <?php endforeach; ?>
                        </div>
                    </div>
                </div>

                <div class="col-md-6">
                    <div class="card">
                        <div class="card-body">
                            <h5 class="card-title">Profile</h5>                          
                            <div class="text-center">
                                <h6><?= session()->get('UserName') ?></h6>
                                <img src="<?= base_url('public/images/uploads/' . session()->get('ProfileImage')) ?>" style="border-radius: 50%; width: 100px; height: 100px;" alt="Profile Image">
                            </div>
                        </div>
                    </div>
                </div>
            </div>
    </div>
</div>
<div class="text-center mt-4">
    <a href="<?= base_url('employee/view') ?>" class="btn btn-outline-primary">Back to Employee List</a>
</div>
</section>
</div>
</main>



<script>
    function changeEntries(entries) {
        // Get all table rows
        var rows = document.querySelectorAll('#employeeTable tbody tr');

        // Hide all rows
        rows.forEach(row => {
            row.style.display = 'none';
        });

        // Show the specified number of entries
        for (var i = 0; i < entries; i++) {
            if (rows[i]) {
                rows[i].style.display = '';
            }
        }
    }
</script>

