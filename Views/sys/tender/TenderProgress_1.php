<!DOCTYPE html>
<html lang="en">
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Employee Table</title>
        <!-- Include DataTables CSS -->
        <link rel="stylesheet" type="text/css" href="https://cdn.datatables.net/1.11.5/css/jquery.dataTables.min.css">
    </head>
    <body>
        <main id="main" class="main">
            <div class="pagetitle">
                <div class="row">

                    <hr style="background-color: black; height: 2px;">
                    <div class="col-8">
                        <h1><img style="margin-right: 10px;" src="data:image/png;base64,"/>VIEW EMPLOYEE</h1>       
                    </div>
                    <div class="col-4">
                        <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">                       
                            <ol class="breadcrumb">
                                <img style="margin-right: 10px;" src="data:image/png;base64,"/>
                                <li class="breadcrumb-item"><a href="index.html">Employee</a></li>
                                <li class="breadcrumb-item active" aria-current="page"><img  style="margin-right: 10px;"  src="data:image/png;base64,"/>View Employee</li>
                            </ol>
                        </nav>   
                    </div>
                    <hr style="background-color: #00001a; height: 2px;">
                </div>
                <!-- End Page Title -->

                <div class="card" style="margin-top: 10px;"> 
                    <div class="card-header" style="background-color: #7070db; color: white;">Employee List</div>
                    <div class="card-body">

                        <div class="card-body">
                            <h5 class="card-title"></h5>
                            <?= form_open('tender/TenderProgress') ?>
                            <div class="row g-3">                              
                                <!-- check -->
                                <!-- Add this code before the table -->

                                <div class="col-md-1">
                                    <label for="entries" class="form-label">Show </label>
                                </div>
                                <div class="col-md-2">
                                    <select style="border-radius: 40px;border: 1px solid  #000066" id="entries" class="form-select" onchange="changeEntries(this.value)">
                                        <option value="5">5</option>
                                        <option value="10" selected>10</option>
                                        <option value="15">15</option>
                                    </select>
                                </div>
                                <div class="col-md-1">
                                    <label for="entries" class="form-label">Designation</label>
                                </div>
                                <div class="col-md-2">

                                </div>
                                <div class="col-md-2">
                                    <input style="border-radius: 40px;border: 1px solid  #000066"  type="text" class="form-control" id="NIC" name="NIC" placeholder="NIC">
                                    <span class="text-danger"><?= service('validation')->getError('NIC') ?></span>
                                </div>
                                <div class="col-md-2">
                                    <button style="border-radius: 40px;border: 1px solid  #000066"  type="submit" class="btn btn-primary">
                                        Search
                                        <img src="data:image/png;base64,"/>
                                    </button>
                                </div>
                                <div class="col-md-2">
                                    <a style="border-radius: 40px;border: 1px solid  #000066" type="reset" class="btn btn-secondary"href="<?= site_url('employee/view') ?>">
                                        Reset
                                        <img src="data:image/png;base64,"/>
                                    </a>
                                </div>              
                            </div>
                            <!-- End Multi Columns Form -->
                            <?= form_close() ?>

                        </div>
                    </div>
                </div>

                <!-- Table Variants -->
                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title"></h5>
                        <div class="table-responsive">

     <!--<table class="table table-dark">-->
     <!--<table class="table table-sm  table-coler-#b3d9ff table-bordered" > -->

         <!--<thead style="border-bottom: 2px solid #ff0000;" >-->
                            <table id="employeeTable" class="table table-striped thead-light table-bordered">
                                <thead style="text-align: center; vertical-align: middle; border-bottom: 2px solid #001a66; border-top:  2px solid #001a66">
                                    <tr>
                                        <th scope="col" class="text-center align-middle">Request ID</th>
                                        <th scope="col" class="text-center align-middle">Request Camp</th>
                                        <th scope="col" class="text-center align-middle">Request Date</th>
                                        <th scope="col" class="text-center align-middle">Request Approved Date</th>

                                        <th scope="col" class="text-center align-middle">Procurement Number</th>
                                        <th scope="col" class="text-center align-middle">Title of the Procurement</th>
                                        <th scope="col" class="text-center align-middle">Procurement Method</th>
                                        <th scope="col" class="text-center align-middle">Vote</th>
                                        <th scope="col" class="text-center align-middle">Geographic Location (for the delivery of goods)</th>
                                        <th scope="col" class="text-center align-middle">Bidding Process Start Date & Time</th>
                                        <th scope="col" class="text-center align-middle">DTEC Appointed Date</th>
                                        <th scope="col" class="text-center align-middle">TEC Approved Date</th>
                                        <th scope="col" class="text-center align-middle">Bid Opening Date & Time</th>
                                        <th scope="col" class="text-center align-middle">Bid Validity Date & Time</th>
                                        <th scope="col" class="text-center align-middle">Contract Value in LKR (excluding taxes)</th>
                                        <th scope="col" class="text-center align-middle">Date of Award</th>
                                        <th scope="col" class="text-center align-middle">Awarded Company</th>
                                        <th scope="col" class="text-center align-middle">View Details</th>
                                    </tr>
                                </thead>

                                <tbody>
                                    <?php
                                    $i = 1;
                                    foreach ($list as $key => $result) {
                                        ?>
                                        <tr>
                                            <td style="font-size: 12px;"><?= $result['RequestID'] ?></td>
                                            <td style="font-size: 12px;"><?= $result['CName'] ?></td>
                                            <td style="font-size: 12px;"><?= $result['ReqDate'] ?></td>
                                            <td style="font-size: 12px;"><?= $result['ApprovedDate'] ?></td>
                                            <td style="font-size: 12px;">STF/LOG/C<?= $result['Tenderid'] ?></td>
                                            <td style="font-size: 12px;"><?= $result['Description'] ?></td>
                                            <td style="font-size: 12px;"><?= $result['PType'] ?></td>
                                            <td style="font-size: 12px;"><?= $result['ICategory'] ?></td>
                                            <td style="font-size: 12px;"><?= $result['Location'] ?></td>
                                            <td style="font-size: 12px;"><?= $result['BStartDate'] ?></td>
                                            <td style="font-size: 12px;"><?= $result['MDate'] ?></td>
                                            <td style="font-size: 12px;"><?= $result['BStartDate'] ?></td>

                                            <td style="font-size: 12px;"><?= $result['BOpen'] ?></td>
                                            <td style="font-size: 12px;"><?= $result['BStart'] ?> to <?= $result['BEnd'] ?></td>
                                            <td style="font-size: 12px;"><?= number_format($result['Costesitmate'], 2) ?></td>
                                            <td style="font-size: 12px;"><?= $result['BOpen'] ?></td>
                                            <td style="font-size: 12px;"><?= $result['BOpen'] ?></td>
                                            <td style="font-size: 12px;"><?= $result['BOpen'] ?></td>


                                        </tr>
                                        <?php
                                        $i++;
                                    }
                                    ?>

                                </tbody>
                            </table>
                            <!-- End Table Variants -->

                        </div>
                    </div>
                </div>
            </div>
        </div>

    </main>

    <script>
        function changeEntries(entries) {
            // Get all table rows
            var rows = document.querySelectorAll('#employeeTable tbody tr');

            // Hide all rows
            rows.forEach(row => {
                row.style.display = 'none';
            });

            // Show the specified number of entries
            for (var i = 0; i < entries; i++) {
                if (rows[i]) {
                    rows[i].style.display = '';
                }
            }
        }
    </script>

