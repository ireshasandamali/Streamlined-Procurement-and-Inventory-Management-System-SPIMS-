
<main id="main" class="main">
    <div class="pagetitle">
        <div class="row">
            <hr style="background-color: black; height: 2px;">
            <div class="col-7">
                <h1><img style="margin-right: 10px;" src="data:image/png;base64,"/>VIEW SUBMITTED BID</h1>       
            </div>
            <div class="col-5">
                <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">                       
                    <ol class="breadcrumb">
                        <img style="margin-right: 10px;" src="data:image/png;base64,"/>
                        <li class="breadcrumb-item"><a href="index.html">BID OPENING</a></li>
                        <li class="breadcrumb-item active" aria-current="page"><img  style="margin-right: 10px;"  src="data:image/png;base64,"/>View Submitted Bid</li>
                    </ol>
                </nav>   
            </div>
            <hr style="background-color: #00001a; height: 2px;">
        </div><!-- End Page Title -->
        
        <!-- Tab navigation -->
        <ul class="nav nav-tabs">
            <li class="nav-item">
                <a class="nav-link active" id="openedBidTab" data-bs-toggle="tab" href="#openedBidContent">National Shopping Procedure</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" id="openingBidTab" data-bs-toggle="tab" href="#openingBidContent">National Competitive Bidding</a>
            </li>
        </ul>
        
        <!-- Tab panes -->
        <div class="tab-content">
            <div class="tab-pane fade show active" id="openedBidContent">
                <div class="card" style="margin-top: 10px;"> 
                    <div class="card-header" style="background-color: #7070db; color: white;">National Shopping Procedure Announcement</div>
                    <div class="card-body">
                        <h5 class="card-title"></h5>                           
                        <!-- Tab panes -->
                <!-- Multi Columns Form -->
                <?= form_open('SubmitedBid/OpeningNSBid') ?>
                <div class="row g-3">  
                    <div class="col-md-1">
                        <label for="entries" class="form-label">Show </label>
                    </div>
                    <div class="col-md-2">
                        <select style="border-radius: 40px;border: 1px solid  #000066" id="entries" class="form-select" onchange="changeEntries(this.value)">
                            <option value="5">5</option>
                            <option value="10" selected>10</option>
                            <option value="15">15</option>
                        </select>
                    </div>
                    <div class="col-md-1">
                        <label for="entries" class="form-label">Entries </label>
                    </div>
                    <div class="col-md-2">
                        <input style="border-radius: 40px;border: 1px solid  #000066" type="date" class="form-control" id="BStarts" name="BStarts">
                    </div>               
                    <div class="col-md-2">
                        <button style="border-radius: 40px;border: 1px solid  #000066"  type="submit" class="btn btn-primary">
                            Search
                            <img src="data:image/png;base64,"/>
                        </button>
                    </div>
                    <div class="col-md-2">
                        <a style="border-radius: 40px;border: 1px solid  #000066" type="reset" class="btn btn-secondary"href="<?= site_url('SubmitedBid/OpeningNSBid') ?>">
                            Reset
                            <img src="data:image/png;base64,"/>
                        </a>
                    </div>              
                   </div>  <?= form_close() ?> 
                </div>   
            </div>  
            <!-- Content for opened bid -->
                <!-- Replace this with your existing code for displaying opened bids -->
                
                <!-- national shopping -->
                <div class="card" style="margin-top: 10px;"> 
                    <div class="card-body">
                        <h5 class="card-title">Commandant / Special Task Force, STF Headquarters, No: 223, Bauddhaloka Mawatha, Colombo 07, now invite for sealed bids from eligible bidders for the following procurements to the Special Task Force (STF) Sri Lanka Police.</h5>           
                        <div class="row">                      
                            <div class="container">        
                             <table class="table" id="invitationTable">
                            <thead style="vertical-align: middle; border-bottom: 2px solid #001a66; border-top:  2px solid #001a66">
                                <tr>

                                    <th scope="col">Bid Announcing Period</th>
                                    <th scope="col">Bid Opening Date</th>
                                      <th scope="col">View More</th>
                                      <th></th>
                                  

                                </tr>
                            </thead>
                            <tbody>
                                <?php
                                    $i = 1;
                                    foreach ($lists as $key => $results) {
                                        ?>
                                    <tr>
                                        <td>from <?= $results['BStarts'] ?> To  <?= $results['BEnds'] ?> </td>
                                        <td> <?= $results['BOpens'] ?></td>

                                            <td>
                                            
                                            <?php
                                            if($results['OpenStatus']==1){   
                                            ?>
                                           <a class="btn btn-info btn-sm" href="<?= site_url('bidAnnounce/changeNCBstatuss/' . $results['ids'].'/0') ?>">Open</a>
                                            <?php
                                                
                                            } else {   
                                             ?>
                                            
                                            <a class="btn btn-danger btn-sm" href="<?= site_url('bidAnnounce/changeNCBstatuss/' . $results['ids'].'/1') ?>">Opened</a>
                                            <?php
                                            }
                                            ?>
                                        </td>
                                   
                                    
                                    
                                    
                                    </tr>
                                    <?php
                                    $i++;
                                }
                                ?>

                            </tbody>
                        </table>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="tab-pane fade" id="openingBidContent">
                <div class="card" style="margin-top: 10px;"> 
            <div class="card-header" style="background-color: #7070db; color: white;">National Competitive Bidding Announcement</div>
            <div class="card-body">
                <h5 class="card-title"></h5>                           

                <!-- Multi Columns Form -->
                <?= form_open('SubmitedBid/TenderOpeningReport') ?>
                <div class="row g-3">  
                    <div class="col-md-1">
                        <label for="entries" class="form-label">Show </label>
                    </div>
                    <div class="col-md-2">
                        <select style="border-radius: 40px;border: 1px solid  #000066" id="entries" class="form-select" onchange="changeEntries(this.value)">
                            <option value="5">5</option>
                            <option value="10" selected>10</option>
                            <option value="15">15</option>
                        </select>
                    </div>
                    <div class="col-md-1">
                        <label for="entries" class="form-label">Entries </label>
                    </div>
                    <div class="col-md-2">
                        <input style="border-radius: 40px;border: 1px solid  #000066" type="date" class="form-control" id="BStart" name="BStart">
                    </div>               
                    <div class="col-md-2">
                        <button style="border-radius: 40px;border: 1px solid  #000066"  type="submit" class="btn btn-primary">
                            Search
                            <img src="data:image/png;base64,"/>
                        </button>
                    </div>
                    <div class="col-md-2">
                        <a style="border-radius: 40px;border: 1px solid  #000066" type="reset" class="btn btn-secondary"href="<?= site_url('SubmitedBid/OpeningBid') ?>">
                            Reset
                            <img src="data:image/png;base64,"/>
                        </a>
                    </div>              
                   </div>   
                </div>   
            </div>
                <!-- Content for opening bid -->
                <!-- Replace this with your existing code for displaying opening bids -->
                <div class="card" style="margin-top: 10px;"> 
                    <div class="card-body">
                        <h5 class="card-title">Commandant / Special Task Force, STF Headquarters, No: 223, Bauddhaloka Mawatha, Colombo 07, now invite for sealed bids from eligible bidders for the following procurements to the Special Task Force (STF) Sri Lanka Police.</h5>           
                        <div class="row">                      
                            <div class="container">        
                                <div class="row">
                                
                            <table class="table" id="invitationTable">
                            <thead style="vertical-align: middle; border-bottom: 2px solid #001a66; border-top:  2px solid #001a66">
                                <tr>

                                    <th scope="col">Bid Announcing Period</th>
                                    <th scope="col">Bid Opening Date</th>
                                      <th scope="col">View More</th>
                                      <th></th>
                                  

                                </tr>
                            </thead>
                            <tbody>
                                <?php
                                    $i = 1;
                                    foreach ($list as $key => $result) {
                                        ?>
                                    <tr>
                                        <td>from <?= $result['BStart'] ?> To  <?= $result['BEnd'] ?> </td>
                                        <td> <?= $result['BOpen'] ?></td>

                                            <td>
                                            
                                            <?php
                                            if($result['OpenStatus']==1){   
                                            ?>
                                           <a class="btn btn-info btn-sm" href="<?= site_url('bidAnnounce/changeNCBstatus/' . $result['id'].'/0') ?>">Open</a>
                                            <?php
                                                
                                            } else {   
                                             ?>
                                            
                                            <a class="btn btn-danger btn-sm" href="<?= site_url('bidAnnounce/changeNCBstatus/' . $result['id'].'/1') ?>">Opened</a>
                                            <?php
                                            }
                                            ?>
                                        </td>
                                   
                                    
                                    
                                    
                                    </tr>
                                    <?php
                                    $i++;
                                }
                                ?>

                            </tbody>
                        </table>
                                    <?= form_close() ?>
                                </div>
                            </div>
                           
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</main>

<script>
    // JavaScript to handle tab switching
    document.addEventListener('DOMContentLoaded', function () {
        // Select the default active tab (openedBidTab)
        document.getElementById('openedBidTab').classList.add('active');
        document.getElementById('openedBidContent').classList.add('show', 'active');
        
        // Add event listeners to tab links
        document.getElementById('openedBidTab').addEventListener('click', function () {
            document.getElementById('openedBidTab').classList.add('active');
            document.getElementById('openedBidContent').classList.add('show', 'active');
            
            document.getElementById('openingBidTab').classList.remove('active');
            document.getElementById('openingBidContent').classList.remove('show', 'active');
        });
        
        document.getElementById('openingBidTab').addEventListener('click', function () {
            document.getElementById('openingBidTab').classList.add('active');
            document.getElementById('openingBidContent').classList.add('show', 'active');
            
            document.getElementById('openedBidTab').classList.remove('active');
            document.getElementById('openedBidContent').classList.remove('show', 'active');
        });
    });
</script>


